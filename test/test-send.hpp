...
const size_t TOKEN_SIZE = 79; // JWT トークンのサイズを適切に設定
...
#pragma omp parallel num_threads(GENERATE_RWER_CACHE_THREAD_NUM)
    {
        ...
        while (CACHE_GEN_FLAG)
        {
            ...
            if (RWer_id >= MAX_RWER_NUM_FOR_CACHE)
                break;
    ...
    // startmanager に結果送信
    {
        ...
        int idx = 0;
        memcpy(message + idx, &hostip_, sizeof(uint32_t));
        idx += sizeof(uint32_t);
        // memcpy(message + idx, &execution_time, sizeof(double)); idx += sizeof(double);
        memcpy(message + idx, &RWer_id_all, sizeof(uint32_t));
        idx += sizeof(uint32_t);
...
inline void RandomWalkSystemWorker::executeRandomWalk(std::unique_ptr<RandomWalker> &&RWer_ptr, StdRandNumGenerator &gen)
{
    ...
    while (1)
    {
        ...
        if (graph_.hasVertex(current_node))
        { // 元グラフのデータを参照して RW
            ...
            if (prev_node != INF)
                RWer_ptr->setPrevIndex(graph_.indexOfUV(current_node, prev_node));

            // RW を一歩進める
            if (RWer_ptr->isSended() == true && RWer_ptr->isSetNextIndex() == true)
            { // 他のサーバから送られてきた RWer
            ...
            else if (RWer_ptr->isEnd() || degree == 0)
            { // 寿命切れ もしくは次数 0 なら終了
        ...
        { // キャッシュデータを参照して RW
            ...
            // 現在頂点の次数情報があるか確認
            if (!cache_.hasDegree(current_node))
            { // 次数情報がない (元グラフの他サーバ隣接ノードの初期状態)
            ...
            { // ランダムな隣接ノードへ遷移
                ...
                if (next_node == INF)
                { // index が存在してなかった場合は index とともに送信
                ...
                if (graph_.hasVertex(next_node))
                    RWer_ptr->updateRWer(next_node, graph_.getHostId(next_node), INF, rand_idx, INF);
                else
                    RWer_ptr->updateRWer(next_node, cache_.getHostId(next_node), INF, rand_idx, INF);
            }
        }
    }
}
...
inline void RandomWalkSystemWorker::endRandomWalk(std::unique_ptr<RandomWalker> &&RWer_ptr)
{
    ...
    {
        if (CHECK_RWER_FLAG && RWer_ptr->isSendedAll())
    ...
    while (PROC_MESSAGE_FLAG)
    {
        ...
        for (int i = 0; i < vec_size; i++)
        {
            ...
            { // 終了して送られてきた RWer の処理
                ...
                if (CHECK_RWER_FLAG)
                    checkRWer(std::move(RWer_ptr_vec[i]));
                else if (MAIN_EX)
                    RW_manager_.setEndTime(RWer_ptr_vec[i]->getRWerID());
            }
...
void RandomWalkSystemWorker::sendMessage()
{
    ...
    while (1)
    {
        ...
        while (idx < vec_size)
        {
            ...
            std::cout << "Added RWer data to message. Total message length: " << now_length << std::endl;
...
// トークンサイズを計算する関数 (仮実装)
uint32_t calculateTokenSize(const char *token_ptr)
{
    // 実際にはトークンの長さに応じた計算を行うべきです
    // ここでは簡単に72バイトの固定サイズとしています
    return 79;
}

void RandomWalkSystemWorker::receiveMessage(const uint16_t &port_num)
{
    ...
    while (1)
    {
        ...
        int received_bytes = recv(sockfd, message, MESSAGE_MAX_LENGTH_RECV, 0);

        /////追加
        if (received_bytes <= 0)
        {
            std::cerr << "Error receiving message or connection closed. received_bytes: " << received_bytes << std::endl;
            continue;
        }

        std::cout << "Received message, size: " << received_bytes << std::endl;
        ///////
        ...
        else if ((ver_id & MASK_MESSEGEID) == RWERS)
        {
            ...
            //////////追加
            std::string secret_key = "your_secret_key";
            std::cout << "認証の検証を行う, RWer_count: " << RWer_count << std::endl;
            for (int i = 0; i < RWer_count; i++)
            {

                // RWer のデータを抽出
                std::cout << "データを抽出します " << std::endl;
                ...
                std::cout << "tokenを抜きます " << std::endl;
                // トークンのサイズを決定し、トークンを抽出
                // uint32_t token_size = calculateTokenSize(message + idx); // トークンのサイズを決定する関数
                // uint32_t token_size = 79;
                // std::string token(message + idx, token_size);
                // idx += token_size;
                // トークンのサイズを固定値で計算した場合の処理
                ...
                //     continue; // トークンの検証に失敗した場合は次の `RWer` へ
                // }
                ...
                // ここから検証を行う
                //  // 動的トークンサイズの処理
                //  uint32_t token_size = calculateTokenSize(message + idx); // トークンサイズを動的に計算
                //  std::string token_str(message + idx, token_size);
                //  idx += token_size;
                ...
                // else
                // {
                //     std::cerr << "トークンの検証に失敗しました, token: " << token << std::endl;
                //     continue;
                // }

                // if (idx + sizeof(RandomWalker) > received_bytes)
                // {
                //     std::cerr << "Error: RandomWalker extraction exceeds message size. idx: " << idx << std::endl;
                //     break;
                // }
            }
            std::cout << "認証を終えた" << std::endl;
        ...
        {
            std::cerr << "Invalid message ID: " << static_cast<int>(ver_id) << std::endl;
            exit(1);
        }
    }
}
...
inline void RandomWalkSystemWorker::sendToStartManager()
{
    ...
    std::this_thread::sleep_for(std::chrono::seconds(5));
    {
        ...
        memcpy(message + idx, &hostip_, sizeof(uint32_t));
        idx += sizeof(uint32_t);
        memcpy(message + idx, &end_count, sizeof(uint32_t));
        idx += sizeof(uint32_t);
        memcpy(message + idx, &execution_time, sizeof(double));
        idx += sizeof(double);
        memcpy(message + idx, &re_send_count, sizeof(uint32_t));
        idx += sizeof(uint32_t);
        send(sockfd, message, sizeof(message), 0); // 送信
