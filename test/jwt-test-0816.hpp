/*
メモリのアクセスなどを変更してできそうだができないコード
『』の中を全て消したらいけるけど行けないっl*/
...
void RandomWalkSystemWorker::sendMessage()
{
    ...
    while (1)
    {
        ...
        while (idx < vec_size)
        {
            ...
            // メッセージサイズ制限を超えた場合、送信
            if (now_length + RWer_data_length + token_length >= MESSAGE_MAX_LENGTH_SEND - sizeof(ver_id) - sizeof(RWer_count))
            {
                std::cout << "total 長さ " << idx << std::endl;
                send_func();
            }
            std::cout << "0 " << now_length << std::endl;
            // // 1. Tokenの長さをメッセージに詰める
            // std::memcpy(message + sizeof(ver_id) + sizeof(RWer_count) + now_length, &token_length, sizeof(token_length));
            // now_length += sizeof(token_length);
            // //  sizeof (token_length) 4バイト
            // std::cout << "1 " << now_length << std::endl;

            // tokenの中身をつめる
            std::memcpy(message + sizeof(ver_id) + sizeof(RWer_count) + now_length, token.c_str(), token_length);
            now_length += token_length;
            // token_length   79バイト
            std::cout << "2 " << now_length << std::endl;
            ...
            std::cout << "3 " << now_length << std::endl;
...
inline void RandomWalkSystemWorker::receiveMessage(const uint16_t &port_num)
{
    ...
    while (1)
    {
        ...
        else if ((ver_id & MASK_MESSEGEID) == RWERS)
        {
            ...
            std::vector<std::unique_ptr<RandomWalker>> RWer_ptr_vec;
            // ループを始める
            std::cout << "start-roooooooooooooooooop" << RWer_count << std::endl;
            for (int i = 0; i < RWer_count; i++)
            {
                ...
                std::cout << "RWer_count: " << i << std::endl;
                // // トークンを抽出
                // std::string token(message + idx, message + idx + TOKEN_SIZE);
                // idx += TOKEN_SIZE;

                // 2. Token を取得
                uint32_t token_length = 79;
                std::string token(message + idx, 79);
                idx += sizeof(token_length);
                ...
                idx += RWer_ptr_vec.back()->getRWerSize(); // RWer データのサイズ
